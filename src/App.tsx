...
import type { ComplianceFinding, ComplianceResult, ComplianceRule, CoachPurpose, PresenterPhrase, PresenterProfile, Product, RuleDocument, RuleUnit, TranscriptSegment } from './shared/types';
...
function OperationsApp() {
  ...
  const load = useCallback(async () => {
    try {
      const [nextRules, nextDocuments, packageState] = await Promise.all([client.publicRuleCandidates(), client.ruleDocuments(), client.rulePackages()]);
  ...
  return <div className="v2-app v2-operations-app"><section className="v2-review-workspace v2-operations-workspace">
    ...
    <main>
      <section className="v2-operations-summary"><span>规则文档 {documents.length}</span><span>待审核语义规则 {units.filter((unit) => unit.status === 'pending_review').length}</span><span>公共规则候选 {rules.length}</span></section>
