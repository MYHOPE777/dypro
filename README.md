...
实时响应采用三层保护：内置/直播间高置信 `warning` 和 `blocked` 规则立即预警；相同话术在 30 秒内复用已完成的方舟结果；其余语义判断并发调用方舟 Responses API。若控制台已为支持的模型开通“在线推理（低延迟）”，可将 `ARK_SERVICE_TIER` 设为官方值 `fast`，否则保持 `auto`；Fast 未开通时服务端会按官方降级语义回退到 `auto`。当前 `doubao-seed-2-1-pro-260628` 不在官方列出的 Fast 支持系列中，不能仅修改参数强行启用。
