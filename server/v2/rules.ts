...
import type { ComplianceResult, ComplianceRule, ComplianceRuleScope, Product, RiskLevel, RuleAuditEntry } from '../../src/shared/types';
...
export class RuleModule {
  ...
  active(roomId: string, product?: Product): ComplianceRule[] {
    return this.store.listRules(roomId, true).filter((rule) => {
  ...
  create(roomId: string, actorId: string, draft: RuleDraft): ComplianceRule {
    ...
    const rule: ComplianceRule = { id: `rule-${randomUUID()}`, roomId, scope, ...(scope === 'product' && draft.productId ? { productId: draft.productId } : {}), ...(scope === 'category' && draft.category ? { category: draft.category } : {}), name: draft.name.trim(), matchType: draft.matchType ?? 'contains', pattern: draft.pattern.trim(), risk: draft.risk, title: draft.title.trim(), reason: draft.reason.trim(), alternative: draft.alternative.trim(), policyRef: draft.policyRef.trim(), enabled: true, status: 'published', version: 1, origin: 'manual', confidence: 1, evidenceCount: 1, evidenceRoomIds: [roomId], createdBy: actorId, approvedBy: actorId, createdAt: now, updatedAt: now };
