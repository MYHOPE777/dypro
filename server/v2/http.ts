...
function boundedBodyString(value: unknown, name: string, maximum: number): string {
  const text = bodyString(value, name);
  if (text.length > maximum) throw new Error(`${name}不能超过 ${maximum} 个字符`);
  return text;
}

function boundedOptionalBodyString(value: unknown, name: string, maximum: number): string | undefined {
...
export function createV2Http(runtime: V2Runtime, options: { clientDir?: string } = {}): V2Http {
  ...
  app.post('/api/v2/sessions/:sessionId/commands', async (request, response) => {
    try {
      ...
      await runtime.dispatch(routeParam(request, 'sessionId'), request.body.command, actorId(request));
  ...
  app.post('/api/v2/sessions/:sessionId/transcripts/:segmentId/speaker', (request, response) => {
    try {
      ...
      return response.json(runtime.review.assignSpeaker(routeParam(request, 'sessionId'), routeParam(request, 'segmentId'), speaker, boundedOptionalBodyString(request.body?.speakerId, '说话人 ID', 96), actorId(request), boundedOptionalBodyString(request.body?.speakerName, '说话人姓名', 80)));
    } catch (error) { return jsonError(response, error); }
  });
  app.post('/api/v2/sessions/:sessionId/transcripts/:segmentId/annotate', (request, response) => {
    try {
      ...
      const selectedText = boundedBodyString(request.body?.selectedText, '选中文本', 2_000);
      ...
      return response.status(201).json(runtime.review.annotateTranscript(sessionId, routeParam(request, 'segmentId'), {
        ...
        ...(boundedOptionalBodyString(request.body?.title, '违规标题', 160) ? { title: boundedOptionalBodyString(request.body?.title, '违规标题', 160) } : {}),
        ...(boundedOptionalBodyString(request.body?.reason, '判断说明', 500) ? { reason: boundedOptionalBodyString(request.body?.reason, '判断说明', 500) } : {}),
        ...(boundedOptionalBodyString(request.body?.alternative, '替代表达', 500) ? { alternative: boundedOptionalBodyString(request.body?.alternative, '替代表达', 500) } : {}),
        ...(boundedOptionalBodyString(request.body?.policyRef, '规则依据', 200) ? { policyRef: boundedOptionalBodyString(request.body?.policyRef, '规则依据', 200) } : {}),
      }, actorId(request)));
    } catch (error) { return jsonError(response, error); }
  });
  ...
  wsServer.on('connection', (socket, request) => {
    ...
    socket.on('message', async (data, isBinary) => {
      ...
      try {
        ...
        await runtime.dispatch(current!.sessionId, command, current.identity.actorId);
